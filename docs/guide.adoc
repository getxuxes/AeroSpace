...
* Every container has an orientation (Possible values: `horizontal`, `vertical`)
...
Use xref:commands.adoc#center-floating[center-floating] to center a floating window on its monitor and optionally size it relative to the monitor:
...
AeroSpace can animate tiled windows when the layout changes (`move`, `resize`, `split`, `layout`, etc.)
...
[animations]
    enabled = true
    duration-ms = 120
    curve = 'ease-out' # or 'spring'
----

Windows are moved step by step via the Accessibility API, so resizing heavy apps may stutter.
Switching between tiling and floating (`layout floating`, `layout tiling`), `center-floating` and entering and leaving
`fullscreen` are animated too.
Switching workspaces and moving floating windows are not animated.

`curve`:

* `ease-out` (default): ease-out cubic over `duration-ms`.
* `spring`: a critically damped spring (no overshoot) that settles in about `duration-ms`. When an animation is interrupted
by another command, the window keeps its current velocity instead of starting over.
macOS's own window tiling animation follows the same kind of curve over about 300 ms.
