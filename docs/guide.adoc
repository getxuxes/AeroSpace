...
* `spring` (default): a critically damped spring (no overshoot) that settles in about `duration-ms`. When an animation is
interrupted by another command, the window keeps its current velocity instead of starting over.
* `ease-out`: ease-out cubic over `duration-ms`.
